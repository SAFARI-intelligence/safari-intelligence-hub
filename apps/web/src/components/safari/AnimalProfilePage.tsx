...
const MOCK_ANIMAL = {
  ...
  story:
    "Simba claimed leadership of the Marsh Pride three seasons ago after a contested challenge at the Mara River crossing. Known for his distinctive black mane and a notch on his left ear from a buffalo encounter in 2023. His territory spans 47 km² across the northern conservancy.",
  recentEvents: [
    { type: "hunt", label: "Successful hunt", zone: "Northern Plains", time: "2h ago", icon: "🦁" },
    {
      type: "migrate",
      label: "Crossed Mara River",
      zone: "River Crossing",
      time: "Yesterday",
      icon: "🌊",
    },
    ...
    {
      type: "alert",
      label: "Territory marking",
      zone: "Eastern Boundary",
      time: "3 days ago",
      icon: "⚡",
    },
  ],
      ...
      {events.map((ev, i) => (
        <div
          ...
          style={{
            ...
            border:
              i === 0 ? `1px solid rgba(201,168,76,0.15)` : `1px solid rgba(255,255,255,0.04)`,
            borderRadius: 8,
          }}
        >
          <span style={{ fontSize: 20, lineHeight: 1.2 }}>{ev.icon}</span>
          <div style={{ flex: 1 }}>
            <div style={{ color: SAFARI_TOKENS.cream, fontSize: 13, fontWeight: 600 }}>
              {ev.label}
            </div>
...
export default function AnimalProfilePage({ animal = MOCK_ANIMAL }) {
  ...
  return (
    <div
      style={{
        minHeight: "100vh",
        background: SAFARI_TOKENS.black,
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
          ...
          <span style={{ color: SAFARI_TOKENS.muted, fontSize: 11 }}>{animal.swahili}</span>
        </div>
        ...
        >
          ...
          <FollowButton
            followers={animal.followers}
            followed={followed}
            onToggle={() => setFollowed((f) => !f)}
          />
        </div>
      ...
      >
        {[
          ...
          >
            <div style={{ fontSize: 12, fontWeight: 700, color: SAFARI_TOKENS.cream }}>
              {f.value}
            </div>
            <div
              style={{
        ...
        {tabs.map((tab) => (
          <button
            ...
            style={{
              ...
              borderBottom:
                activeTab === tab ? `2px solid ${SAFARI_TOKENS.gold}` : "2px solid transparent",
      ...
      <div style={{ padding: "24px", animation: "fadeUp 0.3s ease" }}>
        {activeTab === "story" && (
          <div>
            ...
            >
              Wildlife Story · AI-Generated from Ranger Reports
            </div>
            <p
              style={{
                lineHeight: 1.75,
            ...
            >
              🌿 <strong style={{ color: SAFARI_TOKENS.green }}>Conservation note:</strong>{" "}
              Following Simba funds Marsh Pride monitoring. KSh 240 contributed this month by
              followers.
            </div>
          </div>
        )}
        ...
        {activeTab === "timeline" && (
          <div>
            ...
            >
              Recent Activity · Ranger Validated
            </div>
        ...
        {activeTab === "zone" && (
          <div>
            ...
            >
              Zone Tracking · 2-Hour Delay Enforced
            </div>
            <TrackingMapPlaceholder tier={animal.trackingTier} />
            <div
              style={{ marginTop: 14, fontSize: 11, color: SAFARI_TOKENS.muted, lineHeight: 1.6 }}
            >
              Zone locations are generalized for wildlife safety. Exact GPS coordinates are never
              exposed. Real-time updates available on{" "}
              <strong style={{ color: SAFARI_TOKENS.gold }}>Simba Pro</strong>.
            </div>
          </div>
        )}
        ...
        {activeTab === "adopt" && (
          <div>
            ...
            >
              Adopt Simba · Support the Marsh Pride
            </div>
            {[
              {
                tier: "Supporter",
                price: "KSh 500/mo",
                perks: "Monthly story update + Simba Points",
                highlight: false,
              },
              {
                tier: "Guardian",
                price: "KSh 1,500/mo",
                perks: "Weekly updates + zone access + adoption certificate",
                highlight: true,
              },
              {
                tier: "Pride Partner",
                price: "KSh 4,000/mo",
                perks: "All Guardian + ranger field notes + quarterly report",
                highlight: false,
              },
            ].map((opt, i) => (
              <div
                ...
                >
                  <div>
                    <div
                      style={{
                        fontWeight: 700,
                        color: opt.highlight ? SAFARI_TOKENS.gold : SAFARI_TOKENS.cream,
                        fontSize: 14,
                      }}
                    >
                      {opt.tier} {opt.highlight && "⭐"}
                    </div>
                    <div style={{ fontSize: 11, color: SAFARI_TOKENS.muted, marginTop: 4 }}>
                      {opt.perks}
                    </div>
                  </div>
                  <div
      ...
      {followed && (
        ...
        >
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: SAFARI_TOKENS.gold }}>
              +50 Simba Points earned
            </div>
