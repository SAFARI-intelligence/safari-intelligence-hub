...
import { Star, Calendar, Users, ArrowRight, Loader2 } from "lucide-react";
import { Shell } from "@/components/safari/Shell";
import { MaasaiDivider } from "@/components/safari/MaasaiDivider";
import { packages, swahiliProverbs, images } from "@/lib/safari-data";
...
function BookPage() {
  ...
  const proverb = swahiliProverbs[proverbIdx];
      ...
      <div className="mx-auto max-w-7xl space-y-8">
        <header>
          ...
          <p className="text-muted-foreground mt-2 max-w-2xl">
            Curated premium safaris. Pay in Kenyan Shillings, USD, or any major currency.
          </p>
        </header>
                  ...
                  <div className="mt-auto pt-3 border-t border-border/60">
                    <div className="flex items-end justify-between">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-[var(--maasai)] text-[var(--maasai-foreground)] font-semibold text-sm hover:scale-[1.02] transition shadow-lg"
                      >
          ...
          {loading && (
            ...
            >
              <motion.div
                ...
                animate={{ scale: 1, y: 0 }}
                exit={{ scale: 0.9 }}
                className="glass-strong rounded-3xl p-8 sm:p-10 max-w-md text-center"
              >
                <Loader2 className="h-8 w-8 mx-auto animate-spin text-[var(--gold)]" />
                <MaasaiDivider className="my-6" />
                <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    <p className="text-sm text-muted-foreground mt-3 italic">— {proverb.en}</p>
                  </motion.div>
                </AnimatePresence>
                <p className="text-[11px] uppercase tracking-wider text-muted-foreground mt-6">
                  Confirming with operator · {selected}
                </p>
              </motion.div>
            </motion.div>
          )}
