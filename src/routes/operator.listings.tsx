    ...
    >
      ...
      >
        <div>
          ...
          <p className="op-sub">
            Optimise each listing for AI recommendation weight. Score ≥ 80 = 3× more itinerary
            inclusions.
          </p>
        </div>
        <Link to="/operator/listings/new" className="op-btn op-btn-primary">
      ...
      <section className="op-card">
        <div style={{ display: "flex", alignItems: "flex-start", gap: 24, flexWrap: "wrap" }}>
          <div style={{ minWidth: 180 }}>
            ...
            <div className="op-h2" style={{ marginTop: 4 }}>
              {primary.name}
            </div>
            <div className={`op-score ${scoreClass(score.total)}`} style={{ marginTop: 14 }}>
              {score.total}
              <span style={{ fontSize: 14, color: "var(--op-muted)" }}> / 100</span>
            </div>
          </div>
            ...
            >
              {dimensions.map((d) => (
                ...
                >
                  ...
                  >
                    {statusLabel(d.value, d.max)}
                  </span>
                </li>
              ))}
            </ul>
            <div className="op-sub" style={{ marginTop: 12, fontStyle: "italic" }}>
              Your listing score affects AI recommendation weight. Operators with score ≥ 80 receive
              3× more itinerary inclusions.
            </div>
          </div>
        </div>
      </section>
          ...
          return (
            <article key={l.id} className="op-card" style={{ padding: 0, overflow: "hidden" }}>
              <div
                style={{
                  aspectRatio: "16/9",
                  background: "linear-gradient(135deg, #d8c9a3, #b89968)",
                  display: "grid",
                  placeItems: "center",
              ...
              <div style={{ padding: 14 }}>
                ...
                >
                  ...
                  <span
                    className={`op-pill ${s.total >= 80 ? "op-pill-confirmed" : s.total >= 60 ? "op-pill-pending" : "op-pill-arriving"}`}
                  >
                    {s.total}/100
                  </span>
                </div>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                  ...
                  <button className="op-btn op-btn-ghost">
                    <ExternalLink size={12} /> View on Safari
                  </button>
                </div>
              </div>
            </article>
