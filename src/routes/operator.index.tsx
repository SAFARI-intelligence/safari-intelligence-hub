  ...
  const kpis = [
    {
      ...
      value: formatKsh(monthGmv),
      delta: "↑ 18% vs Apr",
      deltaClass: "op-delta-up",
    },
    {
    ...
    {
      label: "Net to operator (88%)",
      value: formatKsh(netCommission),
      delta: "↑ 14% vs Apr",
      deltaClass: "op-delta-up",
    },
    {
      label: "Average booking value",
      value: formatKsh(avgValue),
      delta: "→ flat vs Apr",
      deltaClass: "op-delta-flat",
    },
  ];
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <Link to="/operator/listings" className="op-btn op-btn-secondary">
            Manage listings
          </Link>
          <Link to="/operator/wildlife" className="op-btn op-btn-primary">
            + Submit ranger report
          </Link>
        </div>
      </header>
      ...
      {alertOpen && (
        <div className="op-alert op-alert-info">
          ...
          <span>
            <strong>3 new Safari AI itineraries</strong> included your property this week. Add 5+
            photos to your Family Tent listing to increase booking conversion by 3×.
          </span>
          <Link to="/operator/listings" className="op-link op-link-tail">
            Update listing →
          </Link>
          <button
      ...
      <div className="op-grid-kpi">
        {kpis.map((k) => (
          <div key={k.label} className="op-card op-card-tight">
            ...
            <div className="op-kpi-value" style={{ marginTop: 8 }}>
              {k.value}
            </div>
            <div className={k.deltaClass} style={{ marginTop: 4 }}>
              {k.delta}
            </div>
          </div>
        ))}
      ...
      <div className="op-grid-6040">
        <section className="op-card">
          ...
          >
            ...
            <Link to="/operator/bookings" className="op-link">
              View all →
            </Link>
          </div>
      ...
      <div className="op-grid-5050">
        <section className="op-card">
          ...
          <div className="op-sub" style={{ marginTop: 6 }}>
            {completed} of {onboarding.length} complete — finish setup to unlock full AI
        ...
        <section className="op-card">
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <Link to="/operator/wildlife" className="op-link">
              Full view →
            </Link>
          </div>
          <ul
                ...
                <li key={r.id} style={{ display: "flex", gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 500 }}>
                      {r.zoneLabel} · {r.title}
                      {r.status === "restricted" && (
                        <span className="op-pill op-pill-internal" style={{ marginLeft: 8 }}>
                          Internal only
                        </span>
                      )}
                    </div>
