...
function BookingsPage() {
  ...
  const totalCommission = filtered.reduce(
    (s, b) => s + (b.status === "cancelled" ? 0 : commissionOf(b.value)),
    0,
  );
  ...
  return (
    ...
    >
      <header>
        ...
        <p className="op-sub">
          View, filter and export every booking sent to your property by SAFARI AI.
        </p>
      </header>

      <div
        className="op-card op-card-tight"
      ...
      >
        ...
        <div style={{ position: "relative", flex: 1, minWidth: 180 }}>
          <Search
            size={13}
            style={{ position: "absolute", left: 10, top: 10, color: "var(--op-muted)" }}
          />
          <input
            className="op-input"
          ...
          <tbody>
            {filtered.map((b) => (
              <tr key={b.id} style={{ cursor: "pointer" }} onClick={() => setSelected(b)}>
                ...
                >
                  {formatKsh(commissionOf(b.value))}
                </td>
                <td
                  style={{
                    textAlign: "right",
...
function BookingModal({ booking, onClose }: { booking: OpBooking; onClose: () => void }) {
  ...
  const timeline = [
    ...
    {
      label: "Safari commission deducted",
      done: booking.status === "confirmed" || booking.status === "arriving",
    },
    {
      ...
      <div className="op-modal" onClick={(e) => e.stopPropagation()}>
        ...
        >
          <div>
            ...
            <div className="op-sub">
              Booking {booking.id} · {booking.travellers}{" "}
              {booking.travellers === 1 ? "traveller" : "travellers"} · {booking.origin}
            </div>
          </div>
        ...
        <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <Section title="Itinerary context (read-only)">
            ...
            <Row
              label="Simba Points generated for guest"
              value={`${booking.simbaPoints.toLocaleString()} pts`}
              muted
            />
          </Section>
                ...
                <li key={i} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12 }}>
                  <span
                    style={{
                      ...
                      height: 10,
                      borderRadius: 999,
                      background: t.done ? "var(--op-green)" : "var(--op-border)",
                    }}
                  />
                  <span style={{ color: t.done ? "var(--op-night)" : "var(--op-muted)" }}>
                    {t.label}
                  </span>
                </li>
              ))}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <div className="op-label" style={{ marginBottom: 8 }}>
        {title}
      </div>
...
function Row({
  ...
  return (
    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
      ...
      <span
        style={{
          color: muted ? "var(--op-muted)" : "var(--op-night)",
