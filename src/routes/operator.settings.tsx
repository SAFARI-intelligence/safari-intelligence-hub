    ...
    >
      ...
      <section className="op-card">
        ...
        <div style={{ marginTop: 12, display: "grid", gap: 12, gridTemplateColumns: "1fr 1fr" }}>
          ...
          <Field label="Registration number">
            <input className="op-input" value={reg} onChange={(e) => setReg(e.target.value)} />
          </Field>
        ...
        <div style={{ marginTop: 14, display: "flex", justifyContent: "flex-end" }}>
          <button className="op-btn op-btn-primary" onClick={() => toast.success("Profile saved")}>
            Save profile
          </button>
        </div>
      </section>
      ...
      <section className="op-card">
        ...
        <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 8 }}>
          <Toggle
            label="New booking (email + SMS)"
            checked={notif.booking}
            onChange={(v) => setNotif((n) => ({ ...n, booking: v }))}
          />
          <Toggle
            label="Payout confirmation"
            checked={notif.payout}
            onChange={(v) => setNotif((n) => ({ ...n, payout: v }))}
          />
          <Toggle
            label="Wildlife intel alerts"
            checked={notif.intel}
            onChange={(v) => setNotif((n) => ({ ...n, intel: v }))}
          />
          <Toggle
            label="AI itinerary inclusions"
            checked={notif.ai}
            onChange={(v) => setNotif((n) => ({ ...n, ai: v }))}
          />
        </div>
      </section>
      ...
      <section className="op-card">
        ...
        <Field label="M-Pesa number">
          <input className="op-input" defaultValue={operator.mpesaMasked} />
        </Field>
        <p className="op-sub" style={{ marginTop: 6 }}>
          Or use bank transfer for international payouts.
        </p>
      </section>
        ...
        <>
          <section className="op-card">
            ...
            <p className="op-sub" style={{ marginTop: 4 }}>
              Add additional logins for rangers and reception.
            </p>
          ...
          <section className="op-card">
            ...
            <p className="op-sub" style={{ marginTop: 4 }}>
              SAFARI Wildlife Story API key (read-only)
            </p>
            <div
              className="op-input"
              style={{
