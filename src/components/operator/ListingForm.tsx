...
export function ListingForm({ mode, listing }: { mode: "create" | "edit"; listing?: OpListing }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "grid", gap: 12, gridTemplateColumns: "1fr 1fr" }}>
        <Field label="Type">
          <select
            className="op-select"
            value={type}
            onChange={(e) => setType(e.target.value as OpListing["type"])}
          >
            {["Lodge", "Tented camp", "Tour package", "Day experience", "Transfer"].map((t) => (
      ...
      <Field
        label={`Description (${description.trim().split(/\s+/).filter(Boolean).length} / 50–500 words)`}
      >
        <textarea
      ...
      <div
        style={{ display: "grid", gap: 12, gridTemplateColumns: "1fr 1fr", alignItems: "center" }}
      >
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 12, fontSize: 12 }}>
          {["Game drives", "Meals", "Transfers", "Park fees"].map((opt) => (
            <label key={opt} style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
              <input
                type="checkbox"
                checked={included.includes(opt)}
                onChange={() => toggleIncluded(opt)}
              />
              {opt}
            </label>
          ))}
